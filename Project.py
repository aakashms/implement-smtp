...
def put_content(l):
    ...
    for index, value in enumerate(l[::-1]):
        str_value = 'Date: {}, Subject: {},To: {}, From: {}'.format(value['Date'], value['Subject'], value['To'],
                                                                    value['From'])
...
def compose_mail():
    ...
    send_btn = Button(com, text='Send', bg='blue', fg='white', font=('', 12),
                      command=lambda: send_mail(root_email_entry.get(),
                                                to_entry, cc_entry, bcc_entry, subject_entry, scrolled_text,
...
root_login_button = Button(root_frame, text='Login', font=('', 16), command=lambda:
